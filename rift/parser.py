"""Rift v0.1 — Lark parser wrapper."""

import sys
from pathlib import Path
from lark import Lark, UnexpectedCharacters, UnexpectedToken, UnexpectedEOF

GRAMMAR_PATH = Path(__file__).parent / "rift_v01.lark"
...
def parse_file(path: str | Path) -> tuple:
    """Parse a .rift file. Returns (tree, None) on success or (None, error_str) on failure."""
    path = Path(path)
    try:
...
def parse_string(source: str, source_name: str = "<string>") -> tuple:
    """Parse Rift source from a string. Returns (tree, None) or (None, error_str)."""
    parser = build_parser()
    try:
        tree = parser.parse(source)
        return tree, None
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python parser.py <file.rift>", file=sys.stderr)
        sys.exit(1)
