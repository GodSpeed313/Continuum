...
| `t3_grounding_reference` | Pointer to the T3 cohort re-sample result that grounded the IdentityIntegrity cross-session change-rate threshold (TODO.md open item) and the second A1.6 falsification check on CadenceIntegrity's ±5s grounding. | T3 session artifact / trace |
